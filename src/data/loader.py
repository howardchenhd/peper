...
def set_dico_parameters(params, data, dico, lang):
    """
    Update dictionary parameters.
    """
    if lang in data['dico']:
        assert data['dico'][lang] == dico
    else:
        data['dico'][lang] = dico
    n_words = len(dico)
    ...
    if hasattr(params, 'n_words') and lang in params.n_words:
        assert params.n_words[lang] == n_words
        ...
        if not hasattr(params, 'n_words'):
            params.n_words = {}
        params.n_words[lang] = n_words
...
def load_para_data(params, data):
    ...
    required_para_train = set(params.clm_steps + params.mlm_steps + params.pc_steps \
                            + params.mt_steps + params.bridge_steps)
    ...
    for src, tgt in params.para_dataset.keys():
        ...
        for splt in ['train', 'valid', 'test']:
            ...
            if  (src,tgt) in params.bridge_steps or (src,tgt) in params.mlm_steps:
